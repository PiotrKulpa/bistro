...
class Home extends Component {
  ...
  componentDidMount() {
    //owl carousel slider
    window.$("#cheffs-slider, #service-slider").owlCarousel({
       autoPlay: false,
       items : 2,
 		pagination : false,
    ...
    // gallery animation
    window.$(".zerogrid").mixItUp();

    //Facts Counters Home Page
 	 window.$(".number-counters").appear(function () {
 		window.$(".number-counters [data-to]").each(function () {
 		  var e = window.$(this).attr("data-to");
 		  window.$(this).delay(6e3).countTo({
 			 from: 50,
 			 to: e,
 			 speed: 3e3,
 			 refreshInterval: 50
   ...
   //Our cheffs On About
 	 window.$("#our-cheffs , #news-slider").owlCarousel({
       autoPlay:4000,
       items :3,
 		pagination : false,
 		navigationText :["<i class='fa fa-angle-left'></i>","<i class='fa fa-angle-right'></i>"],
    ...
    window.$("#testinomial-slider").owlCarousel({
      autoPlay: 3000,
